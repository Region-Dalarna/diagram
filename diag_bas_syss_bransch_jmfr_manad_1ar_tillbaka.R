...
diag_bas_syss_per_bransch_manad_jmfr_1ar_tillbaka <- function(
  ...
  bas_syss_df <- hamta_bas_syss_region_kon_sni2007_fodelseregion_prel_manad_scb(
  			...
  			cont_klartext = "sysselsatta efter arbetsställets belägenhet",			 #  Finns: "sysselsatta efter arbetsställets belägenhet", "sysselsatta efter bostadens belägenhet"
  ...
  chart_df <- bas_syss_df %>%
    ...
    pivot_wider(names_from = tid, values_from = `sysselsatta efter arbetsställets belägenhet`) %>% 
  ...
  diagramtitel <- glue("Skillnad i antal sysselsatta 15-74 år i {region_txt} i {manad_slut} år {ar_slut} jämfört med {manad_start} år {ar_start}")
  diagramfil <- glue("bas_syss_{regionfil_txt}_{manad_slut}_ar{ar_slut}_jmfrt_med_{manad_slut}_ar{ar_slut}.png")
