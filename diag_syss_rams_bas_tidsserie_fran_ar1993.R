...
source("g:/skript/peter/temp/hamta_rams_region_inrikesutrikes_kon_tid__scb.R")
...
rams_df <- hamta_ramsregion_inrikesutrikes_kon_tid__scb(
...
diagramtitel <- glue("Sysselsättningsgrad i {region_txt} år {min(chart_df$år)} - {max(chart_df$år)}")
diagramfil <- glue("syss_inr_utr_{regionfil_txt}_ar{min(chart_df$år)}_{max(chart_df$år)}.png")
