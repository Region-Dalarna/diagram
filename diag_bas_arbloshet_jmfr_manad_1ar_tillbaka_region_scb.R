...
c("https://region-dalarna.github.io/utskrivna_diagram/bas_arbloshet_jmfr_Sveriges län_juli_ar2023-2024.png")
  ...
  hamta_region <- if (all(is.na(vald_region))) hamtaAllaLan(F) else hamtakommuner(lan = vald_region, tamedlan = TRUE, tamedriket = TRUE)
  
  # Om man vill dela upp utrikes hanteras det här
  if(dela_upp_utrikes == TRUE){fodelseregion_val = "*"} else {fodelseregion_val = "Totalt"}
  ...
  bas_syss_df <- hamta_bas_arbstatus_region_kon_alder_fodelseregion_prel_manad_scb(
    ...
    fodelseregion_klartext = fodelseregion_val,			#  NA = tas inte med i uttaget,  Finns: "inrikes född", "utrikes född", "totalt"
  ...
  if(dela_upp_utrikes == TRUE){
    diagramfil <- glue("bas_arbloshet_jmfr_{regionfil_txt}_{manad_txt}_ar{ar_start}-{ar_slut}.png")}else{
      diagramfil <- glue("bas_arbloshet_jmfr_totalt_{regionfil_txt}_{manad_txt}_ar{ar_start}-{ar_slut}.png")}
  ...
  gg_obj <- SkapaStapelDiagram(skickad_df = chart_df,
                               ...
                               manual_color = if(dela_upp_utrikes == TRUE){diagramfarger("rus_sex")} else {diagramfarger("rus_sex")[1]},
                               output_mapp = output_mapp,
                               diagram_facet = if(dela_upp_utrikes == TRUE){TRUE} else {FALSE},
                               facet_sort = TRUE,
