diag_foretagarna <- function(region_vekt = "20", # Vilken region skall vi välja
  ...
  # Ingen SCB-hämtning här - data laddas ned direkt från foretagsklimat.se:s
  # publika Excel-export. "here" togs bort - laddades men användes aldrig.
  ...
  # dplyr/tidyr/curl följer med som beroenden till rddiagram/rdverktyg.
  ...
  kommuner = rdverktyg::skapa_kortnamn_lan(rdverktyg::hamtaregion_kod_namn(rdverktyg::hamtakommuner(region_vekt,tamedlan = TRUE,tamedriket = TRUE))$region,byt_ut_riket_mot_sverige = TRUE)
  ...
  # Laddar ned filen till en temporär fil
  tmpfile <- tempfile(fileext = ".xlsx")
  curl::curl_download(url, tmpfile)

  # Tar hem ett grunddataset som används i samtliga
  grunddata_df <- readxl::read_xlsx(tmpfile, sheet = "Sheet1", col_types = "text") |>
    # reshape year columns into rows
    tidyr::pivot_longer(
      cols = tidyselect::matches("^\\d{4}$"),      # matches 2003, 2004, ... whatever years you have
      names_to = "year",
      values_to = "varde"
    ) |>
      dplyr::mutate(year = as.integer(year),
             varde = as.numeric(varde),
             Kommun = rdverktyg::skapa_kortnamn_lan(Kommun)) |>
      # drop years where EVERY kommun is NA, within a given Faktor + Delserie
        dplyr::group_by(Faktor, Delserie, year) |>
          dplyr::filter(!all(is.na(varde))) |>
            dplyr::ungroup() |>
              dplyr::filter(Kommun %in% kommuner)
  # ===================================================================================================
  ...
  if(diag_arbetsstallen==TRUE){
    ...
    arbetsstallen_df <- grunddata_df |>
      dplyr::filter(Faktor == "Arbetsställen",
             Delserie == "Minst en anställd") |>
        dplyr::filter(year == max(year)) |>
          dplyr::select(-Kommungrupp) |>
            dplyr::rename(Arb_stallen = varde)
    ...
    diagramtitel <- paste0("Antal privata arbetsställen per 1000 invånare i ",rdverktyg::skapa_kortnamn_lan(na.omit(unique(arbetsstallen_df$Län)))  ," år " ,max(arbetsstallen_df$year))
    diagramfilnamn <- paste0("antal_arbetsstallen_",rdverktyg::skapa_kortnamn_lan(na.omit(unique(arbetsstallen_df$Län))),".png")
    objektnamn <- c(objektnamn,paste0("arbetsställen_",rdverktyg::skapa_kortnamn_lan(na.omit(unique(arbetsstallen_df$Län)))))

    gg_obj <- rddiagram::SkapaStapelDiagram(
      skickad_df = arbetsstallen_df |>
  ...
  if(diag_nyforetagsamma==TRUE){
    ...
    nyforetagsamma_df <- grunddata_df |>
      dplyr::filter(Faktor == "Nyföretagsamhet",
             Delserie == "Antal nyföretagsamma individer") |>
      dplyr::filter(year == max(year)) |>
      dplyr::select(-Kommungrupp) |>
      dplyr::rename(Antal_nyforetagsamma = varde)
    ...
    diagramtitel <- paste0("Antal nyföretagsamma per 1000 invånare i ",rdverktyg::skapa_kortnamn_lan(na.omit(unique(nyforetagsamma_df$Län)))  ," år " ,max(nyforetagsamma_df$year))
    diagramfilnamn <- paste0("antal_nyforetagsamma_",rdverktyg::skapa_kortnamn_lan(na.omit(unique(nyforetagsamma_df$Län))),".png")
    objektnamn <- c(objektnamn,paste0("nyföretagssamma_",rdverktyg::skapa_kortnamn_lan(na.omit(unique(nyforetagsamma_df$Län)))))

    gg_obj <- rddiagram::SkapaStapelDiagram(
      skickad_df = nyforetagsamma_df |>
        dplyr::mutate(fokus = ifelse(Kommun == "Sverige", 2,
                                      ifelse(Kommun == rdverktyg::skapa_kortnamn_lan(rdverktyg::hamtaregion_kod_namn(region_vekt)$region),1,0))),
      skickad_x_var = "Kommun",
  ...
  if(diag_foretagsamma==TRUE){
    ...
    foretagsamma_df <- grunddata_df |>
      dplyr::filter(Faktor == "Företagsamhet",
             Delserie == "Andel företagsamma individer") |>
      dplyr::filter(year == max(year)) |>
      dplyr::select(-Kommungrupp) |>
      dplyr::rename(Andel_foretagsamma = varde)
    ...
    diagramtitel <- paste0("Andel företagsamma i ",rdverktyg::skapa_kortnamn_lan(na.omit(unique(foretagsamma_df$Län)))  ," år " ,max(foretagsamma_df$year))
    diagramfilnamn <- paste0("foretagsamma_andel_",rdverktyg::skapa_kortnamn_lan(na.omit(unique(foretagsamma_df$Län))),".png")
    objektnamn <- c(objektnamn,paste0("Andel_företagsamma_",rdverktyg::skapa_kortnamn_lan(na.omit(unique(foretagsamma_df$Län)))))

    gg_obj <- rddiagram::SkapaStapelDiagram(
      skickad_df = foretagsamma_df |>
        dplyr::mutate(fokus = ifelse(Kommun == "Sverige", 2,
                                      ifelse(Kommun == rdverktyg::skapa_kortnamn_lan(rdverktyg::hamtaregion_kod_namn(region_vekt)$region),1,0))),
      skickad_x_var = "Kommun",
