...
diag_50proc_lonesumma <- function(output_mapp = "G:/Samhällsanalys/Statistik/Näringsliv/basfakta/",
  ...
  # Ingen SCB-hämtning här - datan kommer från en lokal Excel-fil (NMS-databasen,
  # senaste filen i mappen väljs automatiskt utifrån ändringsdatum).
  if (!requireNamespace("rddiagram", quietly = TRUE)) {
    remotes::install_github("Region-Dalarna/rdpaket", subdir = "packages/rddiagram")
  }
  if (!requireNamespace("openxlsx", quietly = TRUE)) install.packages("openxlsx")
  # stringr följer med som beroende till rddiagram.
  ...
  #kommun_df <- openxlsx::read.xlsx("G:/skript/projekt/data/sarbarhetsanalys/1_nov_23_50_procent_lonesumma.xlsx",sheet=1)
  ...
  kommun_df <- openxlsx::read.xlsx(latest_file,sheet=1)
  ...
  diagramtitel <- stringr::str_wrap(diagramtitel,55)
  diagramfilnamn <- "50proc_lonesumma.png"

  gg_obj <- rddiagram::SkapaStapelDiagram(
    skickad_df = kommun_df,
